...
- [x] [Ansible](https://www.ansible.com/) to manage environment configuration, build and deploy
...
5. Vagrant will now begin setting up on your machine
   ...
   At any point where the build has failed, do a `vagrant reload`, followed by `vagrant provision` to continue attempt where it failed.
...
$ vim /etc/hosts
...
10.1.2.100  api.lumen-react.local
...
```
    
You should now be able to access the local web app by typing http://api.lumen-react.local

Ping test endpoint: http://api.lumen-react.local/v1/services/ping
...
As per all modern website, lumen-react requires these softwares to be installed on the host machine.
...
You should now be able to access the local website by typing http://lumen-react.local.

Login to manage ToDo with email: `email@mail.com` & password: `password`.

You may also register for a new account.

The front-end is powered by React. Learn React from the official documentation [here](https://facebook.github.io/react/).

Webpack is used to transform ES6 syntax to ES5 that allows most or all modern browsers to read the js files.

## Accessing the Database

It is recommended to download and use SequelPro to access the database.

Refer to the `.env` for the credentials.
...
The build.yml ansible playbook already installs Supervisor to manage
